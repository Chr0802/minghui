  ...
  <div class="main user-layout-register">
    ...
    <a-form ref="formRegister" :autoFormCreate="(form)=>{this.form = form}" id="formRegister">
      <a-form-item
        ...
        <a-input size="large" type="text" placeholder="用户姓名"></a-input>
      </a-form-item>
      ...
      <a-row :gutter="16">
        <a-col class="gutter-row" :span="16">
          <a-form-item
            ...
            :fieldDecoratorOptions="{rules: [{ required: true, message: '请输入登录用户名' }]}">
            <a-input size="large" type="text" placeholder="登录用户名">
        ...
        <a-col class="gutter-row" :span="8">
          <a-button
            ...
            @click.stop.prevent="verify">检测用户名</a-button>
        </a-col>
      </a-row>
      ...
      <a-form-item>
        ...
        <router-link class="login" :to="{ name: 'home' }">返回首页</router-link>
      </a-form-item>
...
<script>
  ...
  import { verifyUsername, addUser } from '@/api/login'
  import md5 from 'md5'
    ...
    methods: {
      ...
      handleSubmit() {
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            values.password2 = this.md5(values.password2)
            this.addUser({...values}).then(res=>{
              switch(res.result){
                case 'WRONG USERNAME':
                  this.$message.warn('用户名不可用')
                  this.setFieldsValue({
                    'username':'',
                    'name':'',
                    'password':'',
                  ...
                  break
                case 'UNAUTHORIZED':
                  this.$message.warn('非法用户，您没有注册用户权限')
      ...
      verify(e) {
        ...
        this.form.validateFields(['username'], {force: true},
          (err, values) => {
            if (!err) {
              this.state.verifyBtn = true
              verifyUsername({username: values.username}).then(res => {
                if(res.result){
                  this.$message.success('当前用户名可用!')
                  this.state.usernameFlag = true
                  this.state.verifyBtn = false
                }else{
                  this.$message.warn('当前用户名不可用!')
                  this.state.verifyBtn = false
                }
              }).catch(err => {
                this.state.verifyBtn = false
